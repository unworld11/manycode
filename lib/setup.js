...
const C = {
  orange: '\x1b[38;2;94;227;138m', // brand green (was orange) - unified palette
...
async function run(opts = {}) {
  ...
  write('\n');
  write(`  ${C.dim}───${C.reset} ${C.bold}manycode${C.reset} ${C.dim}─── let's set you up (30 seconds, arrows + enter)${C.reset}\n\n`);
  ...
  try {
    ...
    if (agentOptions.length > 1) {
      ...
      const picked = await select('which agent should `manycode host` start by default?', agentOptions, startIdx);
    ...
    if (process.platform === 'darwin') {
      const mb = await select('show the live code in your menu bar while hosting?', [
        ...
        { label: 'no', hint: 'use `manycode code` instead' },
    ...
    if (!starAsked) {
      ...
      if (ghAuthed) {
        starAsked = true;
        const star = await select('enjoying this? star manycode on github?', [
          { label: 'sure ⭐', hint: 'runs: gh api -X PUT user/starred/unworld11/manycode' },
          { label: 'maybe later', hint: 'github.com/unworld11/manycode' },
        ], 0);
        if (star.label.startsWith('sure')) {
          const r = spawnSync('gh', ['api', '-X', 'PUT', 'user/starred/unworld11/manycode', '--silent'], { stdio: 'ignore' });
          write(r.status === 0
            ? `  ${C.amber}★${C.reset} ${C.dim}thanks!${C.reset}\n`
            : `  ${C.dim}couldn't star (gh api failed) - github.com/unworld11/manycode${C.reset}\n`);
        }
      }
    }
    ...
    write(blank);
    write(row(`host ${agentName}:`.padEnd(18), 'manycode host'));
    write(row('join a friend:'.padEnd(18), 'manycode join CODE'));
    write(row('change this:'.padEnd(18), 'manycode setup'));
    write(blank);
    ...
    config.save(cfg);
    write(`  ${C.dim}skipped - defaults saved, rerun any time with manycode setup${C.reset}\n\n`);
